 ...
 * @copyright   2011 - http://ObjetivoPHP.com
 * @license     Gratuito (Free) http://www.opensource.org/licenses/gpl-license.html
 * @author      Marcelo Castro (ObjetivoPHP)
 * @link        objetivophp@*******.com
...
class BuscadorFullText
{
     ...
     * @return  void
     */
    public function addFiltro(array $filtrar)
    {
        if (!is_array($filtrar)) {
            return false;
        }
    ...
    private function _cadenaFullText($texto)
    {
        ...
        $patron     = array('/\(\*/', '/\)\*/', '/[\+\-\~\<\>]\*/','(á)','(é)','(í)','(ó)','(ú)','(Á)','(É)','(Í)','(Ó)','(Ú)');
        $remplazo   = array('(', ')', '','a','e','i','o','u','A','E','I','O','U');
        $texto      = preg_replace($patron, $remplazo, $texto);
        // Filtrado final
        if (count($this->_filtrarPalabras)) {
            $texto  = preg_replace($this->_filtrarPalabras, '', $texto);
        }
        return $texto;
    }
}